...
export default function ResultsSection({
  ...
  const isNewTopic = !isFollowUp && history && history.length > 0;

  return (
    <section className="relative z-[1] pb-20 px-4">
      <div className="max-w-7xl mx-auto">
              ...
              <>
                ...
                <div className="flex items-center gap-3 mb-4">
                  <button
                    ...
                    className="w-8 h-8 flex items-center justify-center border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] transition-all cursor-pointer"
                    title={historyVisible ? '收起历史对话' : '展开历史对话'}
                  >
                    <svg
                ...
                <AnimatePresence initial={false}>
                  {historyVisible && (
                    ...
                    >
                      {history.map((entry) => (
                        <CollapsedBubble key={entry.id} entry={entry} />
                      ))}
                    </motion.div>
                  )}
                </AnimatePresence>
        ...
        {/* 当前活跃的 QueryBubble + 结果 */}
        <QueryBubble query={query} isFollowUp={isFollowUp} />

        {/* Related Questions — shown right below query */}
        <RelatedQuestions
          queryKey={queryKey}
          stage={stage}
        ...
        {modelAnswers.length > 0 && (
          <FloatingAnswerPanel
            ...
            isComplete={stage === 'summarizing' || stage === 'done'}
          />
        )}

        {/* Summarizing loading state */}
        {stage === 'summarizing' && !summaryResult && (
          <motion.div
